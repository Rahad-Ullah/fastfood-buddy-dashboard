...
export default function EditRestaurantModal({ trigger, item }: any) {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      const res = await myFetch(`/v1/restaurants/${item._id}`, {
        method: "PATCH",
        body: formData,
      });
      ...
      if (res.success) {
        ...
        toast.error(
          (res as any).error[0].message ?? "Failed to update restaurant",
        );
      }
    } catch (err) {
